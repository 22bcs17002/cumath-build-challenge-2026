...
export default function SocialMediaGenerator() {
  ...
  const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>(["instagram carousel"])
  ...
  const openSocialMedia = (platform: string, content: string, title: string) => {
    const urls = {
      ...
      reddit: `https://www.reddit.com/submit?title=${encodeURIComponent(title)}&text=${encodeURIComponent(content)}`,
      twitter: `https://twitter.com/intent/tweet?text=${encodeURIComponent(content)}`,
      "instagram carousel": "https://www.instagram.com/" // Direct share not supported via URL
    }

    if (platform === "instagram carousel") {
      navigator.clipboard.writeText(content);
      toast.info("Content copied! Opening Instagram for you to post.");
    }

    window.open(urls[platform as keyof typeof urls] || "https://google.com", "_blank")
  }
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2 dark:text-white">
                  {(() => {
                    const IconComponent = platformIcons[selectedPost.platform as keyof typeof platformIcons]
                    return IconComponent ? <IconComponent className="w-5 h-5" /> : <FileText className="w-5 h-5" />
                  })()}
                  {(platformNames[selectedPost.platform as keyof typeof platformNames]) || "Social"} Post
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="flex gap-2 pt-4">
                ...
                >
                  ...
                  Post to {platformNames[selectedPost.platform as keyof typeof platformNames] || "Social"}
                </Button>
              </div>
            </>
          )}
